...
class A3CTrainingThread(object):
    def __init__(self,
        ...
        if (thread_index % 2) == 0:
            self.apply_gradients = global_network.optimizer0.apply_gradients(
                zip(compute_gradients, global_network.weights0)
            )

            self.sync = self.local_network.sync_from(global_network.weights0)
            self.lr = global_network.learning_rate_input0

            self.env = gym.make(cfg.env_name0)
        else:
            self.apply_gradients = global_network.optimizer1.apply_gradients(
                zip(compute_gradients, global_network.weights1)
            )

            self.sync = self.local_network.sync_from(global_network.weights1)
            self.lr = global_network.learning_rate_input1

            self.env = gym.make(cfg.env_name1)
        ...
        if thread_index < 2:
            ...
            self.summary_op = tf.summary.scalar('episode_reward', self.score_input)
