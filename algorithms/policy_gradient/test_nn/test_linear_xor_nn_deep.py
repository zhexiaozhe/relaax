...
w2 = tf.Variable(tf.random_uniform(shape=[HIDDEN_SIZE, HIDDEN_SIZE]))
layer2 = tf.nn.relu(tf.matmul(layer1, w2))

# 3rd layer: weight matrix variable & activation
w3 = tf.Variable(tf.random_uniform(shape=[HIDDEN_SIZE, 1]))
if OUTPUT == 'relu':
    nn_output = tf.nn.relu(tf.matmul(layer2, w3))
    # relu more straightforward in this way -> since works better than sigmoid
elif OUTPUT == 'sigmoid':
    nn_output = tf.nn.sigmoid(tf.matmul(layer2, w3))
else:
    nn_output = tf.matmul(layer2, w3)
